...
class TrainDataset(Dataset):
    ...
    def __init__(
        ...
        templist=[]
        for filename in self.noise_dataset_list:
            noise_file = filename
            noise_new_added, sr = sf.read(noise_file, dtype='float32')
            assert sr == self.sr
            templist.append(noise_new_added)

        self.noise_dataset_list = templist
    ...
    def _select_noise_y(self, target_length):
        ...
        while remaining_length > 0:
            noise_new_added = self._random_select_from(self.noise_dataset_list)
            # noise_new_added,sr = sf.read(noise_file, dtype='float32')
            # assert sr == self.sr
    ...
    def snr_mix(self,clean_y, noise_y, snr, target_dB_FS, target_dB_FS_floating_value, rir=None, eps=1e-6):
        ...
        return noisy_y, clean_y , clean_direct
    ...
    def __getitem__(self, index):
        ...
        noisy_y, clean_y, clean_direct = self.snr_mix(
            clean_y=clean_y,
            noise_y=noise_y,
            snr=snr,
        ...
        return noisy_y, clean_direct,clean_file
