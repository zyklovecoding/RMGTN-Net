...
import json
from src.perloss.phoneloss import PerceptualLoss
import pandas as pd
import os
...
class MSTF(nn.Module):
    ...
    def forward(self, x0, x1):
        ...
        x__ = self.project1(x__.permute(0, 3, 2, 1)).permute(0, 3, 2,
                                                             1)
        ...
        return self.project2(out).permute(0, 3, 2, 1)
...
class Multi_GTU(nn.Module):
    def __init__(self, num_of_timesteps, in_channels, time_strides, kernel_size, pool=False):
        ...
        self.pool = pool
        if self.pool:
            self.pooling = torch.nn.MaxPool2d(kernel_size=(1, 2), stride=None, padding=0, return_indices=False,
                                              ceil_mode=False)
            self.fcmy = nn.Sequential(
                nn.Linear(int(3 * num_of_timesteps / 2), num_of_timesteps),
                nn.Dropout(0.05),
            )
        else:
...
def stft_y_mse(perloss: PerceptualLoss, prediction, target, type="train") -> Tensor:
    if type != "train":
        mse_loss = torch.nn.MSELoss(reduction='mean')
        ...
        prediction, yr_hat = prediction
        target, yr = target
        mse_loss = torch.nn.MSELoss(reduction='mean')
        prediction = torch.view_as_real(prediction)
        target = torch.view_as_real(target)
        ploss = perloss(yr_hat, yr).mean()
        loss = mse_loss(prediction, target)
        loss = ploss + loss
    return loss
...
class McNetIO(Module):
    ...
    def __init__(
            ...
            perloss: Optional[Union[Module, List[Module]]] = None,
    ) -> None:
        super().__init__()
    ...
    def prepare_input(self, x: Tensor, *args, **kwargs) -> Dict[str, Any]:
        ...
        X = torch.stft(x, n_fft=self.ft_len, hop_length=self.ft_hop, window=self.window, win_length=self.ft_len,
                       return_complex=True)  # type:ignore
        ...
        # X = torch.view_as_real(X).reshape(batch_size, frame_num, freq_num, chn_num * 2).clone()  # 确保reshape后不共享内存
        # X = X.permute(0, 3, 2, 1).clone()  # 确保permute后不共享内存
        # X = mulgtn(X.clone())  # mulgtn内部可能有inplace操作
        # X = X.permute(0, 3, 2, 1).clone()  # 再次确保不共享内存
        # X = X.reshape(batch_size, frame_num, freq_num, chn_num, 2).clone()  # 确保reshape后不共享内存
        # X = torch.view_as_complex(X)  # 最后恢复为复数
        ...
        if self.use_cumulative_normalization == False:
            ...
            XrMM = cumulative_normalization(original_signal_mag=torch.abs(Xr),
    ...
    def prepare_prediction(self, X: Tensor, output: Tensor, XrMM: Tensor, original_time_len: int, *args,
                           **kwargs) -> Tensor:
        ...
        pred = torch.istft(output, n_fft=self.ft_len, hop_length=self.ft_hop, window=self.window,
                           win_length=self.ft_len, length=original_time_len)  # [B, t]  type:ignore
        return pred

    def prepare_time_domain(self, x: Tensor, prediction: Tensor, XrMM: Tensor, original_time_len: int, *args,
                            **kwargs) -> Tensor:
    ...
    def loss(self, prediction: Tensor, target: Tensor, reduce_batch: bool = False, type="train", *args,
             **kwargs) -> Tensor:
        """loss for prediction and target
        """
        if reduce_batch:
            return self.loss_func(self.perloss, prediction=prediction, target=target, type=type).mean()
        else:
            return self.loss_func(self.perloss, prediction=prediction, target=target, type=type)
...
class cIRMIO(McNetIO):
    ...
    def prepare_target(self, x: Tensor, yr: Tensor, XrMM: Tensor, *args, **kwargs) -> Any:
        ...
        stft_xr = torch.stft(xr, n_fft=self.ft_len, hop_length=self.ft_hop, window=self.window, win_length=self.ft_len,
                             return_complex=False)
        stft_yr = torch.stft(yr, n_fft=self.ft_len, hop_length=self.ft_hop, window=self.window, win_length=self.ft_len,
                             return_complex=False)
    ...
    def prepare_prediction(self, X: Tensor, output: Tensor, XrMM: Tensor, original_time_len: int, *args,
        ...
        prediction = torch.view_as_complex(output.contiguous())  # [B,F,T]
        return prediction
    ...
    def prepare_time_domain(self, x: Tensor, prediction: Tensor, XrMM: Tensor, original_time_len: int, *args,
        ...
        prediction_icIRM = torch.view_as_complex(prediction_icIRM)
        wav = torch.istft(prediction_icIRM, n_fft=self.ft_len, hop_length=self.ft_hop, window=self.window,
                          win_length=self.ft_len, length=original_time_len)
        return wav
...
class RNN_FC(nn.Module):
    ...
    def __init__(
        ...
        self.sequence_model = nn.LSTM(
            ...
            batch_first=True,
            # If batch_first is true, the shape of input tensor should be [batch_size,time_step,feature]. The output is the same.
        ...
        if self.use_FC:
            ...
            if bidirectional:
                self.fc_output_layer = nn.Conv2d(in_channels=hidden_size * 2, out_channels=output_size, kernel_size=(3, 1), stride=1,
                              padding=(1, 0), dilation=1, groups=1, bias=True)
            else:
                self.fc_output_layer = nn.Conv2d(in_channels=hidden_size, out_channels=output_size, kernel_size=(3, 1), stride=1,
                              padding=(1, 0), dilation=1, groups=1, bias=True)
    ...
    def forward(self, x: Tensor,B,T,F) -> Tensor:
        """
        Args:
            x: [B, T, Feature]
        Returns:
...
class McNet(pl.LightningModule):
    ...
    def __init__(
            ...
            all: Optional[Union[Module, List[Module]]] = None,
            mulgtn: Optional[Union[Module, List[Module]]] = None,
            mulgtn2: Optional[Union[Module, List[Module]]] = None,
            mstf: Optional[Union[Module, List[Module]]] = None,
        ...
        self.mulgtn = mulgtn if not isinstance(mulgtn, List) else nn.Sequential(*mulgtn)
        self.mulgtn2 = mulgtn2 if not isinstance(mulgtn2, List) else nn.Sequential(*mulgtn2)
        self.mstf = mstf if not isinstance(mstf, List) else nn.Sequential(*mstf)
        self.all = all if not isinstance(all, List) else nn.Sequential(*all)
        self.io = io
        self.order = order
    ...
    def on_train_start(self):
        ...
        if self.current_epoch == 0:
            if self.trainer.is_global_zero and hasattr(self.logger, 'log_dir') and 'notag' not in self.hparams.exp_name:
                tag_and_log_git_status(self.logger.log_dir + '/git.out', self.logger.version, self.hparams.exp_name,
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Dict[str, Any]]:  # type:ignore
        ...
        for curr in self.order:
            ...
            if curr == 'freq':
                i = i.reshape(B * T, F, -1)
                i = self.freq(i,B,T,F).reshape(B, T, F, -1)
                everyoutput["freq"] = i
            elif curr == 'narr':
                if curr.endswith('+X'):
                ...
                X = data['XrMag'].permute(0, 1, 3, 2).reshape(B * T, -1, F,
                                                                  1)
                if self.hparams.sub_freqs[0] != 0:
                    X = torch.concat(
                        [X[:, :, :self.hparams.sub_freqs[0], :], X, X[:, :, -self.hparams.sub_freqs[0]:, :]],
                        dim=2)
                    Xsub = torch.nn.functional.unfold(X, kernel_size=(
                        self.hparams.sub_freqs[0] * 2 + 1, 1))
                else:
                    Xsub = X.reshape(B * T, -1, F)
                i = data['XMag'].permute(0, 1, 3, 2).reshape(B * T, -1, F, 1)
                i = torch.concat([i[:, :, :self.hparams.sub_freqs[1], :], i, i[:, :, -self.hparams.sub_freqs[1]:, :]],
                                 dim=2)
                ...
                XrMag = torch.nn.functional.pad(XrMag, pad=self.hparams.look_past_and_ahead, mode='constant',
                                                value=0)
                XrMag = XrMag.reshape(B * F, 1, -1, 1)
                XrMag = torch.nn.functional.unfold(XrMag,
                                                   kernel_size=(self.hparams.look_past_and_ahead[0] +
                                                                self.hparams.look_past_and_ahead[1] + 1,
                                                                1))
                XrMag = XrMag.reshape(B, F, -1, T).permute(0, 3, 1, 2).reshape(B * T, F, -1)
                i = torch.cat([i, XrMag], dim=-1)
                i = self.full(i,B,T,F).reshape(B, T, F, -1)
                i = i.permute(0, 3, 1, 2)
                i = self.mulgtn2(i)
                i = i.permute(0, 2, 3, 1)
            elif curr.startswith('twosplit'):
                finalinput = self.mstf(everyoutput["narr"],i)
                finalinput = finalinput.reshape(B*T,F,-1)
                i = self.all(finalinput,B,T,F).reshape(B, T, F, -1)
            else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_time_domain_loss == False:
            ...
            prediction = (prediction, yr_hat)
            target = (target, yr)
            loss = self.io.loss(prediction=prediction, target=target, reduce_batch=True, type="train")
        else:
            yr_hat = self.io.prepare_time_domain(x=x, prediction=prediction, **data)
            loss = self.io.loss(prediction=yr_hat, target=yr, reduce_batch=True, type="val")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        yr_hat = self.io.prepare_time_domain(x=x, prediction=prediction, **data)
        if self.use_time_domain_loss == False:
            loss = self.io.loss(prediction=prediction, target=target, reduce_batch=True, type="val")
        else:
            loss = self.io.loss(prediction=yr_hat, target=yr, reduce_batch=True, type="val")
        ...
        self.log('val/sisdr', SISDR.to(self.device), batch_size=x.shape[0], sync_dist=True)
        self.log('val/nb_pesq', NB_PESQ.to(self.device), batch_size=x.shape[0], sync_dist=True)
        self.log('val/stoi', STOI.to(self.device), batch_size=x.shape[0], sync_dist=True)
        self.log('val/wb_pesq', WB_PESQ.to(self.device), batch_size=x.shape[0], sync_dist=True)

        # self.validation_outputs.append({
        #     'epoch': self.current_epoch,
        #     'batch_idx': batch_idx,
        #     'name' : paras['noisy_wav_path'],
        #     'loss': loss.item(),
        #     'SDR': SDR.item()
        # })
        self.log('val/' + self.io.loss_name, loss.to(self.device), batch_size=x.shape[0], sync_dist=True)
        self.log('val/sdr', SDR.to(self.device), batch_size=x.shape[0], sync_dist=True)
        self.log('val/criteria', CRITERIA.to(self.device), batch_size=x.shape[0], logger=False, sync_dist=True)
        return loss

    # def on_predict_epoch_end(self) -> None:
    #     if len(self.validation_outputs) == 0:
    #         print("⚠️ No validation outputs to save.")
    #         return
    #     print(sum(self.validation_outputs)/(864*2))
    #     # 5️⃣ 清除 self.validation_outputs 以防止下个 epoch 污染
    #     self.validation_outputs.clear()

    # def on_validation_epoch_end(self):
    #     """
    #     在每个验证 epoch 结束时，将验证的结果保存到 CSV 文件
    #     """
    #     # 1️⃣ 将 self.validation_outputs 转换为 DataFrame
    #     if len(self.validation_outputs) == 0:
    #         print("⚠️ No validation outputs to save.")
    #         return
    #
    #     # 2️⃣ 将结果转换为 DataFrame
    #     df = pd.DataFrame(self.validation_outputs)
    #
    #     # 3️⃣ 确定 CSV 文件的路径
    #     log_dir = "/home/doyourthing_zyk/cepnet/McNet/valcsv/"
    #     os.makedirs(log_dir, exist_ok=True)  # 确保目录存在
    #     csv_path = os.path.join(log_dir, f'val_results_epoch_{self.current_epoch}.csv')
    #
    #     # 4️⃣ 将 DataFrame 保存为 CSV
    ...
    #
    #     print(f"✅ Validation results for epoch {self.current_epoch} saved to {csv_path}")
    #
    #     # 5️⃣ 清除 self.validation_outputs 以防止下个 epoch 污染
    #     self.validation_outputs.clear()
    ...
    def test_step(self, batch, batch_idx):
        ...
        metrics, input_metrics, imp_metrics = cal_metrics_functional(metric_list=metric_list, preds=yr_hat, target=yr,
    ...
    def predict_step(self, batch: Tensor, batch_idx: Optional[int] = None,
        ...
        x, paras = batch
        # 创建保存目录
        self.modelname = "mc2split"
        self.modelname = os.path.join("/home/doyourthing_zyk/predictwav/",self.modelname)
        self.exp_save_path = self.modelname
        os.makedirs(self.exp_save_path, exist_ok=True)
        # 默认exist_ok: False, 在目标目录已存在的情况下触发FileExistsError异常，但如果exist_ok是True,则在目标目录已经存在的情况下不会触发FileExistsError异常。

        # # 记录开始时间
        # start_time = tttime.time()
        #
        # # 前向传播
        prediction, data = self.forward(x)
        #
        # # 记录结束时间
        # end_time = tttime.time()
        #
        # # 计算处理时间
        # processing_time = end_time - start_time
        #
        #
        # self.validation_outputs.append(processing_time)
        yr_hat = self.io.prepare_time_domain(x=x, prediction=prediction, **data)
...
if __name__ == "__main__":
    with torch.no_grad():
        io = McNetIO([0, 1, 2, 3, 4, 5], ref_channel=4, loss_func=stft_y_mse, ft_len=512, ft_hop=256,
                     sliding_window_len=192, use_cumulative_normalization=True)
        model = McNet(
            ...
            sub=RNN_FC(input_size=327, output_size=64, hidden_size=384, num_layers=1, bidirectional=False,
                       act_funcs=('', 'ReLU'), use_FC=True),
            full=RNN_FC(input_size=70, output_size=2, hidden_size=128, num_layers=1, bidirectional=True,
                        act_funcs=('', ''), use_FC=True),
