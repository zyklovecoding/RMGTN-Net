...
from McNet.src.util.acoustic_utils import norm_amplitude, tailor_dB_FS, is_clipped, load_wav, subsample
...
class TrainDataset(Dataset):
    ...
    def __init__(
            ...
            nchannels: int = 6,
            ref_channel: int = 5,
            selected_channels: List[int] = [0, 1, 2, 3, 4, 5],
    ):
        """
        ...
        self.clean_dataset_list = [os.path.join(os.path.abspath(os.path.expanduser(clean_dataset_dir)), cp) for cp in
                                   clean_dataset_list]
        self.noise_dataset_list = [os.path.join(os.path.abspath(os.path.expanduser(noise_dataset_dir)), np) for np in
                                   noise_dataset_list]
        ...
        rir_dir = "/home/doyourthing_zyk/data/RIR/rirs/"
        rir_direct = "/home/doyourthing_zyk/data/RIR/rir_clean/"
        self.rir_dir = [os.path.join(rir_dir, f) for f in os.listdir(rir_dir)]
        self.rir_direct = [os.path.join(rir_direct, f) for f in os.listdir(rir_direct)]
    ...
    def apply_multi_channel_rir(self, clean_y, rir):
        """
    ...
    def snr_mix(self, clean_y, noise_y, snr, target_dB_FS, target_dB_FS_floating_value, rir=None, eps=1e-6):
        """
        ...
        if rir is not None:
            rirlist = rir[0]
            rirfilepath = random.choice(rirlist)
            rircleanpath = os.path.join("/home/doyourthing_zyk/data/RIR/rir_clean/", os.path.basename(rirfilepath))
            rir = loadmat(rirfilepath)['hIRs'].T
            rir_clean = loadmat(rircleanpath)['hIRd'].T
            clean_direct = self.apply_multi_channel_rir(np.squeeze(clean_y, axis=0), rir_clean)
            clean_y = self.apply_multi_channel_rir(np.squeeze(clean_y, axis=0), rir)
        ...
        clean_direct_rms = (clean_direct ** 2).mean() ** 0.5
        ...
        noisy_target_dB_FS = np.random.randint(target_dB_FS - target_dB_FS_floating_value,
        ...
        if is_clipped(noisy_y):
            ...
            ornoise_y = ornoise_y / noisy_scalar
            clean_direct = clean_direct / noisy_y_scalar

        return noisy_y, clean_y, clean_direct , ornoise_y
    ...
    def __getitem__(self, index):
        ...
        clean_y = clean_y.T
        assert sr == self.sr
        clean_y = np.expand_dims(clean_y, axis=0)
